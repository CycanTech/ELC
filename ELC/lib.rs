...
mod ELC {
    ...
    mod tests {
        ...
        type Event = <Elc as ::ink_lang::BaseEvent>::Type;
        ...
        fn assert_transfer_event(
            ...
            let expected_topics = vec![
                encoded_into_hash(b"Elc::Transfer"),
        ...
        fn transfer_works() {
            ...
            assert_eq!(elc.transfer(accounts.bob, 10), Ok(()));
            // Bob owns 10 tokens.
            assert_eq!(elc.balance_of(accounts.bob), 10);
        ...
        fn invalid_transfer_should_fail() {
            ...
            assert_eq!(
                elc.transfer(accounts.eve, 10),
                Err(Error::InsufficientBalance)
            );
            // Alice owns all the tokens.
            assert_eq!(elc.balance_of(accounts.alice), 100);
            assert_eq!(elc.balance_of(accounts.bob), 0);
            assert_eq!(elc.balance_of(accounts.eve), 0);
        ...
        fn transfer_from_works() {
            ...
            assert_eq!(
                elc.transfer_from(accounts.alice, accounts.eve, 10),
                Err(Error::InsufficientAllowance)
            );
            // Alice approves Bob for token transfers on her behalf.
            assert_eq!(elc.approve(accounts.bob, 10), Ok(()));
            ...
            // Eve owns tokens.
            assert_eq!(elc.balance_of(accounts.eve), 10);
        ...
        fn allowance_must_not_change_on_failed_transfer() {
            ...
            // Alice approves Bob for token transfers on her behalf.
            let alice_balance = elc.balance_of(accounts.alice);
            let initial_allowance = alice_balance + 2;
            assert_eq!(elc.approve(accounts.bob, initial_allowance), Ok(()));
            ...
            assert_eq!(
                elc.transfer_from(accounts.alice, accounts.eve, alice_balance + 1),
                Err(Error::InsufficientBalance)
            );
            // Allowance must have stayed the same
            assert_eq!(
                elc.allowance(accounts.alice, accounts.bob),
                initial_allowance
            );
